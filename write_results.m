function write_results(grid,Q,R,iter,case_name)
    ...
    for i=1:grid.N_vertices
        fprintf(file,'%0.10f\t%0.10f\t%0.10f\n',grid.vertices(i,1),grid.vertices(i,2),grid.vertices(i,3));
    end
    fprintf(file,'\t\t\t\t</DataArray>\n');
    fprintf(file,'\t\t\t</Points>\n');
    fprintf(file,'\t\t\t<Cells>\n');
    ...
    for i=1:grid.N_faces
        fprintf(file,'%i\t%i\t%i\n',grid.faces(i,1)-1,grid.faces(i,2)-1,grid.faces(i,3)-1);
    end
    fprintf(file,'\t\t\t\t</DataArray>\n');
